...
namespace NMib
{
	...
	namespace NDataProcessing
	{
			...
			case EUniversallyUniqueIdentifierGenerate_Random:
				{
					...
					NMem::fg_MemCopy((uint8 *)this, &Random, 4);
					Random = NMisc::fg_GetRandomUnsigned();
					NMem::fg_MemCopy((uint8 *)this + 4, &Random, 4);
					Random = NMisc::fg_GetRandomUnsigned();
					NMem::fg_MemCopy((uint8 *)this + 8, &Random, 4);
					Random = NMisc::fg_GetRandomUnsigned();
					NMem::fg_MemCopy((uint8 *)this + 12, &Random, 4);
		...
		void CUniversallyUniqueIdentifier::fp_CreateFromSHA1(CUniversallyUniqueIdentifier const &_Namespace, void const *_pData, mint _DataLen)
		{
			...
			NDataProcessing::CHashDigest_SHA1 Digest = Hash;
		...
		void CUniversallyUniqueIdentifier::fp_CreateFromHash(uint8 *_pHash, int32 _Version)
		{
			...
			NMem::fg_MemCopy(this, _pHash, sizeof(*this));
		...
		bool CUniversallyUniqueIdentifier::operator == (CUniversallyUniqueIdentifier const &_Right) const
		{
			if (m_TimeLow != _Right.m_TimeLow)
				return false;
			if (m_TimeMid != _Right.m_TimeMid)
				return false;
			if (m_TimeHiAndVersion != _Right.m_TimeHiAndVersion)
				return false;
			if (m_ClockSequenceHiAndReserved != _Right.m_ClockSequenceHiAndReserved)
				return false;
			if (m_ClockSquenceLow != _Right.m_ClockSquenceLow)
				return false;
			if (m_Node[0] != _Right.m_Node[0])
				return false;
			if (m_Node[1] != _Right.m_Node[1])
				return false;
			if (m_Node[2] != _Right.m_Node[2])
				return false;
			if (m_Node[3] != _Right.m_Node[3])
				return false;
			if (m_Node[4] != _Right.m_Node[4])
				return false;
			if (m_Node[5] != _Right.m_Node[5])
				return false;
			return true;
		}
		...
		bool CUniversallyUniqueIdentifier::operator < (CUniversallyUniqueIdentifier const &_Right) const
		{
			if (m_TimeLow < _Right.m_TimeLow)
				return true;
			else if (m_TimeLow > _Right.m_TimeLow)
				return false;
			if (m_TimeMid < _Right.m_TimeMid)
				return true;
			else if (m_TimeMid > _Right.m_TimeMid)
				return false;
			if (m_TimeHiAndVersion < _Right.m_TimeHiAndVersion)
				return true;
			else if (m_TimeHiAndVersion > _Right.m_TimeHiAndVersion)
				return false;
			if (m_ClockSequenceHiAndReserved < _Right.m_ClockSequenceHiAndReserved)
				return true;
			else if (m_ClockSequenceHiAndReserved > _Right.m_ClockSequenceHiAndReserved)
				return false;
			if (m_ClockSquenceLow < _Right.m_ClockSquenceLow)
				return true;
			else if (m_ClockSquenceLow > _Right.m_ClockSquenceLow)
				return false;
			if (m_Node[0] < _Right.m_Node[0])
				return true;
			else if (m_Node[0] > _Right.m_Node[0])
				return false;
			if (m_Node[1] < _Right.m_Node[1])
				return true;
			else if (m_Node[1] > _Right.m_Node[1])
				return false;
			if (m_Node[2] < _Right.m_Node[2])
				return true;
			else if (m_Node[2] > _Right.m_Node[2])
				return false;
			if (m_Node[3] < _Right.m_Node[3])
				return true;
			else if (m_Node[3] > _Right.m_Node[3])
				return false;
			if (m_Node[4] < _Right.m_Node[4])
				return true;
			else if (m_Node[4] > _Right.m_Node[4])
				return false;
			if (m_Node[5] < _Right.m_Node[5])
				return true;
			else if (m_Node[5] > _Right.m_Node[5])
				return false;
			return false;
		}

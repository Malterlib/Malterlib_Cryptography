			...
			(
				[&]() -> decltype(auto)
				{
					...
					if (!_SignOptions.m_OverrideSubjectCommonName.f_IsEmpty())
					{
						...
						ERR_clear_error();
						if (!X509_NAME_add_entry_by_txt(X509_get_subject_name(pCertificate), "CN", MBSTRING_UTF8, (unsigned char const *)_SignOptions.m_OverrideSubjectCommonName.f_GetStr(), -1, -1, 0))

...
namespace NMib::NCryptography
{
	namespace
	{
		...
		NConcurrency::TCFuture<NContainer::CByteVector> fg_RequestTimestamp
			(
				NConcurrency::TCActor<NWeb::CHttpClientActor> _HttpClientActor
			...
			)
		{
			...
			NWeb::CHttpClientActor::CRequest Request;
			Request.m_URL = fg_Move(_TimestampURL);
			Request.m_Method = NWeb::CHttpClientActor::EMethod_POST;
			Request.m_Headers["Content-Type"] = "application/timestamp-query";
			Request.m_SendData = fg_Move(RequestData);

			auto Result = co_await _HttpClientActor(&NWeb::CHttpClientActor::f_SendRequest, fg_Move(Request));
		...
		NConcurrency::TCFuture<NEncoding::CEJsonSorted> fg_SignDetachedAsync(CSignParams _Params)
		{
			...
			auto fAddToDigest = [&](void const *_pData, umint _Len)
				{
					if (!_Len)
						return;
			...
			{
				if (_Params.m_pAborted->f_Load(NAtomic::gc_MemoryOrder_Relaxed))
			...
			if (!_Params.m_TimestampURLs.f_IsEmpty())
			{
				...
				NConcurrency::TCActor<NWeb::CHttpClientActor> HttpClientActor{fg_Construct(), "Timestamp curl actor"};

				auto HttpClientActorDestroy = co_await NConcurrency::fg_AsyncDestroy(HttpClientActor);
						...
						NConcurrency::g_Dispatch(BlockingActor) / [FilePath, pAborted]() mutable
						{
							...
							SignParams.m_Manifest = NFile::CDirectoryManifest::fs_GetManifest
								(
									...
									, [&]
									{
										if (pAborted->f_Load(NAtomic::gc_MemoryOrder_Relaxed))

...
namespace NMib
{
	namespace NDataProcessing
	{
		...
		public:
			...
			void f_Clear()
			{
				NMem::fg_ObjectSet(mp_Data, 0, t_Size);
			}
			...
			{
				NMem::fg_MemCopy(mp_Data, _Src.mp_Data, sizeof(mp_Data));
			}
			...
			{
				return NMem::fg_MemCmp(mp_Data, _Other.mp_Data, sizeof(mp_Data));
			}

			bint operator == (const TCMessageDigest &_Src) const
			{
				if (NMem::fg_MemCmp(mp_Data, _Src.mp_Data, t_Size))
					return false;
			...
			bint operator < (const TCMessageDigest &_Src) const
			{
				return NMem::fg_MemCmp(mp_Data, _Src.mp_Data, t_Size) < 0;
			}
...
//				DMibSafeCheck(!(((mint)_pData) & (DMibAlignmentOf(CData) - 1)), "Must be correct alignment");
			...
			void fp_AddData(const void *_pData, mint _Len)
			{
				...
				if (m_iBlockPos)
				{
					if (m_iBlockPos + _Len >= EBlockSizeBytes)
					{
						...
						NMem::fg_MemCopy(m_Block + m_iBlockPos, pData, ToCopy);
						pData += ToCopy;
						_Len -= ToCopy;
				...
				else
				{
					...
					{
						DMibSafeCheck(m_iBlockPos == 0, "Must not have saved block");
						NMem::fg_MemCopy(m_Block, pData+EBlockSizeBytes*0, EBlockSizeBytes);
						fp_AddBlock(m_Block);
						NMem::fg_MemCopy(m_Block, pData+EBlockSizeBytes*1, EBlockSizeBytes);
						fp_AddBlock(m_Block);
						NMem::fg_MemCopy(m_Block, pData+EBlockSizeBytes*2, EBlockSizeBytes);
						fp_AddBlock(m_Block);
						NMem::fg_MemCopy(m_Block, pData+EBlockSizeBytes*3, EBlockSizeBytes);
						fp_AddBlock(m_Block);
						pData += EBlockSizeBytes*4;
						_Len -= EBlockSizeBytes*4;
					}
					...
					{
						DMibSafeCheck(m_iBlockPos == 0, "Must not have saved block");
						NMem::fg_MemCopy(m_Block, pData, EBlockSizeBytes);
						fp_AddBlock(pData);
						pData += EBlockSizeBytes;
						_Len -= EBlockSizeBytes;
					}
				...
				NMem::fg_MemCopy(m_Block + m_iBlockPos, pData, _Len);
				m_iBlockPos += _Len;
			}
		...
		public:
			...
			{
				NMem::fg_MemCopy(m_Block, _Src.m_Block, sizeof(m_Block));
			...
			~TCHashImpl()
			{
				NMem::fg_MemClear(m_BlockAligned);
				NMem::fg_MemClear(m_State);
			}
			...
			static CMessageDigest fs_DigestFromData(const NContainer::TCVector<uint8> &_Data)
			{
				CMessageDigest Ret;
				TCHashImpl Hash;
	...
	namespace NStream
	{
		...
		class TCBinaryStreamTypeReference<t_CStream, NDataProcessing::TCMessageDigest<t_Size, t_CHash> >
		{
		public:
			static void fs_Feed(t_CStream &_Stream, NDataProcessing::TCMessageDigest<t_Size, t_CHash> const &_Data)
			{
				_Stream.f_FeedBytes(_Data.f_GetData(), t_Size);
			}
			static void fs_Consume(t_CStream &_Stream, NDataProcessing::TCMessageDigest<t_Size, t_CHash> &_Data)
			{

...
namespace NMib
{
	namespace NCryptography
	{
		...
		TCBinaryStream_Encrypted<t_CParentStream, t_CStreamType>::TCBinaryStream_Encrypted
			(
			 	NNet::CEncryptKeyIV const &_KeyIV
			 	, NNet::ESSLDigest _HMAC
			 	, NContainer::CSecureByteVector const &_HMACKey
			)
			: mp_KeyIV(_KeyIV)
			, mp_HMAC(_HMAC)
			, mp_HMACKey(_HMACKey)
			, mp_BlockSize(NNet::CEncryptKeyIV::fs_GetBlockSize(_KeyIV.m_Crypto))
		{
		...
		void TCBinaryStream_Encrypted<t_CParentStream, t_CStreamType>::f_Open(tf_CParentStream &&_ParentStream, NFile::EFileOpen _OpenFlags)
		{
			...
			{
			case NNet::ESSLCrypto_DES_EDE3_CBC:
			case NNet::ESSLCrypto_AES_128_CBC:
			case NNet::ESSLCrypto_AES_256_CBC:
				mp_bCanChangePosition = true;
				mp_bIsCBC = true;
				break;
			case NNet::ESSLCrypto_AES_128_ECB:
			case NNet::ESSLCrypto_AES_256_ECB:
				mp_bCanChangePosition = true;
				mp_bIsECB = true;
				break;
			}
			...
			else if (_OpenFlags == NFile::EFileOpen_Read)
			{
				...
				mp_pEncryptContext = fg_Construct(NNet::ESSLCryptoFlags_Decrypt | NNet::ESSLCryptoFlags_UsePadding, mp_KeyIV);
				mint HMACSize = 0;
				if (mp_HMAC != NNet::ESSLDigest_None)
				{
				...
				if ((mp_FileLen - HMACSize) % mp_BlockSize || mp_FileLen < HMACSize + mp_BlockSize)
				...
				if (mp_FileLen > 0)
				{
					...
					if (mp_HMAC != NNet::ESSLDigest_None)
					{
						...
						{
							aint ThisTime = fg_Min(mp_EncryptedFileLen - mp_FilePos, (mint)mp_BufferSize);
					...
					if (mp_bCanChangePosition)
					{
						...
						if (Pos >= 0)
						{
							...
							NNet::CIncrementalEncrypt TempDecryptionContext
								(
								 	NNet::ESSLCryptoFlags_Decrypt | NNet::ESSLCryptoFlags_UsePadding
		...
		void TCBinaryStream_Encrypted<t_CParentStream, t_CStreamType>::fp_WriteDirty(bool _bFinalize)
		{
			...
			if (mp_bCurrentDirty)
			{
				...
				DMibCheck(Size <= mp_DecryptedBlock.f_GetLen());
		...
		template <typename t_CParentStream, typename t_CStreamType>
		aint TCBinaryStream_Encrypted<t_CParentStream, t_CStreamType>::fp_PrepareBlock(NStream::CFilePos _Pos, bint _bWrite)
		{
			if (mp_CurrentLoaded >= 0 && _Pos >=  mp_CurrentLoaded && _Pos < mp_CurrentLoaded + mp_BufferSize)
			{
			...
			auto &ParentStream = fp_GetParentStream();
			NStream::CFilePos BlockPos = _Pos % mp_BufferSize;
			...
			if (_Pos + 1 > mp_FileLen)
			{
				if (_bWrite)
				{
					...
					NMem::fg_MemClear(mp_DecryptedBlock.f_GetArray(), mp_BufferSize + mp_BlockSize);
				}
				else
			...
			{
				aint ThisTime = fg_Min(mp_EncryptedFileLen - mp_CurrentLoaded, (mint)mp_BufferSize);
				aint DecryptedBytes = 0;
				...
				if (mp_CurrentLoaded != mp_LastLoaded)
				{
					if (mp_CurrentLoaded > 0)
					{
						...
						if (mp_bIsCBC)
						{
							...
							ParentStream.f_ConsumeBytes(IV.f_GetArray(mp_BlockSize), mp_BlockSize);
							NNet::CEncryptKeyIV KeyIV{mp_KeyIV.m_Key, IV, mp_KeyIV.m_Crypto};
							mp_pEncryptContext = fg_Construct(NNet::ESSLCryptoFlags_Decrypt | NNet::ESSLCryptoFlags_UsePadding, KeyIV);
						}
						else
						{
		...
		void TCBinaryStream_Encrypted<t_CParentStream, t_CStreamType>::f_FeedBytes(const void *_pMem, mint _nBytes)
		{
			...
			while (_nBytes)
			{
				...
				aint ThisTime = fg_Min(_nBytes, (mint)mp_BufferSize - Pos);
				DMibCheck(Pos + ThisTime <= mp_DecryptedBlock.f_GetLen());
				NMem::fg_MemCopy(mp_DecryptedBlock.f_GetArray() + Pos, pMem, ThisTime);
		...
		void TCBinaryStream_Encrypted<t_CParentStream, t_CStreamType>::f_ConsumeBytes(void *_pMem, mint _nBytes)
		{
			...
			if (mp_FilePos + _nBytes > mp_FileLen)
				DMibErrorCryptography("Read past end of file");
			...
			while (_nBytes)
			{
				...
				aint ThisTime = fg_Min(_nBytes, (mint)mp_BufferSize - Pos);
				DMibCheck(Pos + ThisTime <= mp_DecryptedBlock.f_GetLen());
				NMem::fg_MemCopy(pMem, mp_DecryptedBlock.f_GetArray() + Pos, ThisTime);
		...
		template <typename t_CParentStream, typename t_CStreamType>
		void TCBinaryStream_Encrypted<t_CParentStream, t_CStreamType>::f_SetBufferSize(NStream::CFilePos _BufferSize)
		{
			if (mp_OpenFlags != NFile::EFileOpen_None)
